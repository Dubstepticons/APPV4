...
class PositionService:
    ...
    def save_open_position(
        ...
        try:
            ...
            if success:
                log.info(
                    f"[PositionService] Open position saved: {symbol} {qty}@{entry_price} "
                    f"(mode={mode}, account={account})"
                )
            else:
                log.error(
                    f"[PositionService] Failed to save open position: {symbol} "
                    f"(mode={mode}, account={account})"
                )
    ...
    def get_open_position(self, mode: str, account: str):
        """
        Fetch the open position for the given mode/account scope.

        Returns the repository dict or None if nothing is recorded.
        """
        try:
            return self._repo.get_open_position(mode=mode, account=account)
        except Exception as e:
            log.error(f"[PositionService] Error reading open position: {e}", exc_info=True)
            return None
