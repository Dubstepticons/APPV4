...
class PositionDisplay:
    ...
    def __init__(
        ...
        symbol_banner=None,
        live_banner=None,
        indicators: Optional["VisualIndicators"] = None,
    ):
        """
        ...
        # Share indicator helper (avoid re-instantiating per refresh)
        if indicators is not None:
            self._indicator_helper = indicators
        else:
            from panels.panel2.visual_indicators import VisualIndicators

            self._indicator_helper = VisualIndicators()
    ...
    def _update_heat_cell(
        ...
        """
        from panels.panel2.visual_indicators import format_heat_time
        ...
        helper = self._indicator_helper
        color_name = helper.get_heat_color(heat_duration)
        should_flash = helper.should_flash_heat(heat_duration)
    ...
    def _update_stop_cell(self, state: "PositionState") -> None:
        ...
        """
        from panels.panel2.visual_indicators import get_stop_proximity_color
        ...
        if state.stop_price is not None:
            ...
            # Check proximity
            helper = self._indicator_helper
            should_flash = helper.should_flash_stop(state)
    ...
    def _update_vwap_cell(self, state: "PositionState", metrics: dict) -> None:
        """Update VWAP cell (actual VWAP price from CSV)."""
        # Display actual VWAP price (not distance)
        if state.vwap != 0:
            # Format VWAP to 2 decimal places
            text = f"{state.vwap:.2f}"
            ...
            if state.last_price and state.last_price != 0:
                ...
                vwap_dist = state.last_price - state.vwap
                ...
                # No last price, use neutral color
                color = THEME.get("text_primary", "#E6F6FF")
