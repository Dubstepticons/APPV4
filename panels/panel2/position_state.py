...
class PositionState:
    ...
    def with_trade_extremes(self, price: float) -> PositionState:
        """
        Update trade min/max prices from a new market price.
        """
        if not self.has_position():
            return self
        new_min = price if self.trade_min_price == 0 else min(self.trade_min_price, price)
        new_max = price if self.trade_max_price == 0 else max(self.trade_max_price, price)
        return replace(
            self,
            trade_min_price=new_min,
            trade_max_price=new_max,
    ...
    def with_symbol(self, symbol: str) -> PositionState:
        """
        Update symbol (immutable - returns new instance).
        """
    ...
    def with_target(self, price: Optional[float]) -> PositionState:
        """Update target price only."""
        return replace(self, target_price=price)

    def with_stop(self, price: Optional[float]) -> PositionState:
        """Update stop price only."""
        return replace(self, stop_price=price)
    ...
    def with_position(
        ...
        """
        Update core position attributes (immutable - returns new instance).
        """
        return replace(
            self,
            entry_qty=entry_qty,
    ...
    def with_scope(
        ...
        """
        Update mode/account scope (immutable - returns new instance).
        """
        return replace(
            self,
            current_mode=mode or self.current_mode,
            current_account=self.current_account if account is None else account,
        )

    def with_entry_time(self, timestamp: int) -> PositionState:
        """Update entry time epoch (immutable)."""
        return replace(self, entry_time_epoch=timestamp)
    ...
    def from_position_domain(cls, position, market_data: dict) -> PositionState:
        ...
        if isinstance(position, dict):
            pos_dict = position
            entry_qty = pos_dict.get("qty", 0)
            entry_price = pos_dict.get("entry_price", 0.0)
            is_long = pos_dict.get("side", "LONG").upper() == "LONG" if pos_dict.get("side") else (entry_qty > 0)
            entry_time_epoch = int(pos_dict.get("entry_time").timestamp()) if isinstance(pos_dict.get("entry_time"), datetime) else 0
            target_price = pos_dict.get("target_price")
            stop_price = pos_dict.get("stop_price")
            trade_min_price = pos_dict.get("trade_min_price")
            trade_max_price = pos_dict.get("trade_max_price")
            entry_vwap = pos_dict.get("entry_vwap")
            entry_cum_delta = pos_dict.get("entry_cum_delta")
            entry_poc = pos_dict.get("entry_poc")
            mode = pos_dict.get("mode", "SIM")
        ...
        return cls(
            ...
            # Trade extremes
            trade_min_price=trade_min_price,
