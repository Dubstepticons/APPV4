...
class Panel1(ThemeAwareMixin, QtWidgets.QWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self._mode_balances: Dict[str, Optional[float]] = {"SIM": None, "LIVE": None, "DEBUG": None}
        self._mode_pnl_cache: Dict[str, Dict[str, Optional[float]]] = {
            mode: {"value": None, "pct": None, "up": None} for mode in self._mode_balances
        }
        self._signal_bus = None  # Populated in _connect_signal_bus
    ...
    def _init_modules(self) -> None:
        ...
        if vb is not None:
            self._hover_handler = HoverHandler(
                ...
                on_hover_end=self._restore_live_display,
                parent=self
            )
    ...
    def _connect_signal_bus(self) -> None:
        ...
        try:
            from core.signal_bus import get_signal_bus  # Local import to avoid circular dependency
            self._signal_bus = get_signal_bus()
        except Exception as exc:
            log.error("[Panel1] Failed to acquire SignalBus", error=str(exc))
            self._signal_bus = None
            return

        bus = self._signal_bus
        bus.balanceUpdated.connect(
            self._on_balance_updated,
            QtCore.Qt.ConnectionType.QueuedConnection,
        )
        bus.modeChanged.connect(
            lambda mode: self.set_trading_mode(mode, None),
            QtCore.Qt.ConnectionType.QueuedConnection,
        )
        bus.balanceDisplayRequested.connect(self._on_balance_display_requested)
        bus.equityPointRequested.connect(self._on_equity_point_requested)
    ...
    def _on_balance_display_requested(self, balance: float, mode: str) -> None:
        """Handle balance display events from SignalBus."""
        normalized_mode = self._ensure_mode_cache(mode or "SIM")
        self._mode_balances[normalized_mode] = balance

        if normalized_mode == self._current_mode:
            self.set_account_balance(balance)

    def _on_equity_point_requested(self, balance: float, mode: str) -> None:
        """Handle equity data point events from SignalBus."""
        self.update_equity_series_from_balance(balance, mode=mode)

    def _on_balance_updated(self, balance: float, account: str) -> None:
        """Handle DTC-driven balance updates (primarily LIVE)."""
        try:
            from utils.trade_mode import detect_mode_from_account
        except Exception:
            detect_mode_from_account = None  # type: ignore
        ...
        mode = "SIM"
        if detect_mode_from_account and account:
            with contextlib.suppress(Exception):
                mode = detect_mode_from_account(account)

        if mode == "SIM":
            # SIM balances are driven by PnL, not DTC
            return
    ...
    def _restore_live_display(self) -> None:
        """Restore balance and PnL labels when hover ends."""
        self._apply_cached_state_for_mode(self._current_mode)

    def _update_pnl_for_timeframe(self, points: list[tuple[float, float]], mode: Optional[str] = None) -> None:
        """
        ...
        mode = self._ensure_mode_cache(mode or self._current_mode)
        ...
        pnl_val = pnl_result["amount"]
        pnl_pct = pnl_result["percentage"]
        pnl_up = pnl_result["is_positive"]

        self._mode_pnl_cache[mode] = {"value": pnl_val, "pct": pnl_pct, "up": pnl_up}

        if mode != self._current_mode:
            return

        # Store state for active mode
        self._pnl_val = pnl_val
    ...
    def _apply_cached_state_for_mode(self, mode: str) -> None:
        """Apply cached balance/PnL for the specified mode."""
        normalized_mode = (mode or "SIM").upper()
        self._ensure_mode_cache(normalized_mode)

        balance = self._mode_balances.get(normalized_mode)
        self.lbl_balance.setText(fmt_money(balance) if balance is not None else "--")

        pnl_state = self._mode_pnl_cache.get(normalized_mode, {})
        val = pnl_state.get("value")
        pct = pnl_state.get("pct")
        up = pnl_state.get("up")
        self.set_pnl_for_timeframe(val, pct, up)
    ...
    def _ensure_mode_cache(self, mode: str) -> str:
        """Ensure cache dictionaries have entries for the given mode."""
        normalized = (mode or "SIM").upper()
        if normalized not in self._mode_balances:
            self._mode_balances[normalized] = None
        if normalized not in self._mode_pnl_cache:
            self._mode_pnl_cache[normalized] = {"value": None, "pct": None, "up": None}
        return normalized
    ...
    def set_trading_mode(self, mode: str, account: Optional[str] = None) -> None:
        ...
        """
        mode = self._ensure_mode_cache(mode or "SIM")
        ...
        self._apply_cached_state_for_mode(mode)
    ...
    def set_timeframe(self, tf: str) -> None:
        ...
        Args:
            tf: Timeframe string ("LIVE", "1D", "1W", "1M", "3M", "YTD", "ALL")
        """
    ...
    def set_account_balance(self, balance: Optional[float]) -> None:
        ...
        """
        normalized_mode = self._ensure_mode_cache(self._current_mode)
        self._current_balance = balance
        self._mode_balances[normalized_mode] = balance
    ...
    def update_equity_series_from_balance(self, balance: Optional[float], mode: Optional[str] = None) -> None:
        ...
        self._mode_balances[mode] = balance
        if mode == self._current_mode:
            self.set_account_balance(balance)
        ...
        if mode == self._current_mode:
            ...
            if curve:
                ...
                # Update PnL
                self._update_pnl_for_timeframe(curve, mode=mode)
        else:
            curve = self._equity_state.get_equity_curve(mode, self._current_account)
            if curve:
                self._update_pnl_for_timeframe(curve, mode=mode)
