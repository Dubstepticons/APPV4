...
class HoverHandler(QtCore.QObject):
    ...
    def __init__(
        ...
        on_hover_end: Optional[Callable[[], None]] = None,
    ...
    def _on_mouse_move(self, pos: QtCore.QPointF) -> None:
        ...
        # Snap to timeframe increment and clamp within dataset range
        x_snapped = TimeframeManager.snap_timestamp(self._current_timeframe, x_mouse)
        x_snapped = max(xs[0], min(xs[-1], x_snapped))
        y = self._interpolate_balance(xs, ys, x_snapped)

        # Update state
        self._hovering = True
        self._scrub_x = x_snapped
        ...
        if self._hover_seg:
            ...
            p0 = vb.mapViewToScene(QtCore.QPointF(x_snapped, y_min))
            p1 = vb.mapViewToScene(QtCore.QPointF(x_snapped, y_top))
        ...
        if self._hover_text:
            label = self._format_timestamp(x_snapped)

            # Position timestamp near top of chart
            y_min, y_max = yr[0], yr[1]
            y_span = y_max - y_min
            y_top = y_max - y_span * 0.05 if y_span else y_max

            # Clamp x position to avoid text going off screen
            pad = max((xr[1] - xr[0]) * 0.03, 1.0)
            x_clamped = max(xr[0] + pad, min(xr[1] - pad, x_snapped))
        ...
        self._update_header_for_hover(x_snapped, y)
    ...
    def _interpolate_balance(self, xs: list[float], ys: list[float], target_x: float) -> float:
        """
        Interpolate balance value for an arbitrary timestamp.
        """
        if target_x <= xs[0]:
            return ys[0]
        if target_x >= xs[-1]:
        ...
        idx = bisect.bisect_right(xs, target_x)
    ...
    def _format_timestamp(self, timestamp: float) -> str:
        """
        Format timestamp string according to timeframe rules.
        """
        dt = datetime.fromtimestamp(timestamp)
        tf = self._current_timeframe.upper()
