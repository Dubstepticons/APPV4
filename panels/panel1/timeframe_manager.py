...
class TimeframeManager:
    ...
    - YTD: Year to date (start of current year)
    - ALL: Complete equity history
    """
    ...
    TIMEFRAME_CONFIGS: dict[str, dict[str, Optional[int]]] = {
        ...
        "YTD": {
            "window_sec": None,       # Start of current year
            "snap_sec": 604800        # 1 week
        },
        "ALL": {
            "window_sec": None,       # Entire history
            "snap_sec": 604800        # 1 week
        },
    }

    # Valid timeframe strings
    VALID_TIMEFRAMES = ["LIVE", "1D", "1W", "1M", "3M", "YTD", "ALL"]
    ...
    @classmethod
    def snap_timestamp(cls, timeframe: str, value: float) -> float:
        """
        Snap a timestamp to the configured increment for a timeframe.

        Args:
            timeframe: Timeframe string
            value: Timestamp to snap

        Returns:
            Snapped timestamp
        """
        snap = cls.get_snap_seconds(timeframe)
        if snap <= 1:
            return value
        # Floor to nearest increment to keep scrub consistent
    ...
    def filter_points_for_timeframe(
        ...
        Args:
            ...
            timeframe: Timeframe string ("LIVE", "1D", "1W", "1M", "3M", "YTD", "ALL")
        ...
        # Special-case YTD to use start-of-year boundary
        if timeframe == "YTD":
            ref_time = float(points[-1][0]) if current_time is None else float(current_time)
            dt = datetime.fromtimestamp(ref_time)
            year_start = datetime(dt.year, 1, 1).timestamp()
            start_index = cls._find_window_start_index(points, year_start)
            return points[start_index:]

        # Get window size for timeframe
        window_sec = cls.get_window_seconds(timeframe)

        # No window limit -> ALL timeframe
    ...
    def calculate_x_range(
        ...
        if timeframe == "YTD":
            dt = datetime.fromtimestamp(x_max)
            x_min = datetime(dt.year, 1, 1).timestamp()
