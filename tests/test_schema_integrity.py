...
Automated validation of DTC schemas against real-world message payloads.
This test file validates all Pydantic schemas and database models against:
  1. Test payload examples
  2. Schema field presence and types
  3. Enum value consistency
  4. Database model compatibility

Run with: python -m pytest -q tests/test_schema_integrity.py --disable-warnings
"""
...
from data.schema import AccountBalance, OrderRecord, TradeRecord
...
class TestOrderUpdateSchema(unittest.TestCase):
    ...
    def test_high_low_during_position_coalescing(self):
        """Test high/low during position coalescing"""
        order1 = OrderUpdate.model_validate(
            {
                "Type": 301,
        ...
        self.assertEqual(order1.get_high_during_position(), 5850.0)
        self.assertEqual(order1.get_low_during_position(), 5780.0)
...
class TestPositionUpdateSchema(unittest.TestCase):
    ...
    def test_position_with_pnl(self):
        """Test position with P&L data"""
        payload = {
            "Type": 306,
            "Symbol": "MES",
            "Quantity": 5,
            "AveragePrice": 5800.0,
            "OpenProfitLoss": 250.0,
            "DailyProfitLoss": 100.0,
            "HighPriceDuringPosition": 5850.0,
            "LowPriceDuringPosition": 5750.0,
        }
        pos = PositionUpdate.model_validate(payload)

        self.assertEqual(pos.OpenProfitLoss, 250.0)
        self.assertEqual(pos.DailyProfitLoss, 100.0)
        self.assertEqual(pos.HighPriceDuringPosition, 5850.0)
...
class TestAccountBalanceSchema(unittest.TestCase):
    ...
    def test_account_balance_with_aliases(self):
        """Test that AccountValue and NetLiquidatingValue work"""
        # Test with AccountValue
        balance1 = AccountBalanceUpdate.model_validate(
            {
                "Type": 600,
                "AccountValue": 100000.0,
            }
        )
        self.assertEqual(balance1.AccountValue, 100000.0)

        # Test with NetLiquidatingValue
        balance2 = AccountBalanceUpdate.model_validate(
            {
                "Type": 600,
                "NetLiquidatingValue": 100000.0,
            }
        )
        self.assertEqual(balance2.NetLiquidatingValue, 100000.0)
...
class TestDatabaseSchemaCompatibility(unittest.TestCase):
    """Test database models can be instantiated"""

    def test_trade_record_creation(self):
        """Test TradeRecord can be created with minimal fields"""
        trade = TradeRecord(
            symbol="MESZ24",
            side="LONG",
            qty=2,
        ...
        self.assertEqual(trade.symbol, "MESZ24")
    ...
    def test_order_record_creation(self):
        """Test OrderRecord can be created"""
        order = OrderRecord(
            order_id="ORD-001",
            symbol="MESZ24",
            side="BUY",
    ...
    def test_account_balance_record_creation(self):
        """Test AccountBalance can be created"""
        balance = AccountBalance(
            account_id="120005",
            balance=100000.0,
        )

        self.assertEqual(balance.account_id, "120005")
        self.assertEqual(balance.balance, 100000.0)
...
class TestParsingRobustness(unittest.TestCase):
    ...
    def test_parse_with_null_values(self):
        """Test parsing with null/None values"""
        payload = {
            "Type": 301,
            "ServerOrderID": "TEST",
            "Symbol": None,
...
class TestSchemaValidationErrors(unittest.TestCase):
    ...
    def test_order_update_with_invalid_quantity_type(self):
        """Test that invalid quantity types are handled"""
        payload = {
            "Type": 301,
            "ServerOrderID": "TEST",
            "OrderQuantity": "not_a_number",  # Invalid
        }
        # Pydantic will attempt coercion or skip field
        order = OrderUpdate.model_validate(payload)
        self.assertIsInstance(order, OrderUpdate)

    def test_position_update_with_invalid_quantity_type(self):
        """Test PositionUpdate with invalid quantity"""
        payload = {
            "Type": 306,
    ...
    def test_enum_value_normalization(self):
        ...
        # BuySell should be int
        order = OrderUpdate.model_validate(
            {
                "Type": 301,
                "BuySell": 1,
            }
        ...
        self.assertEqual(order.BuySell, 1)
        self.assertIsInstance(order.BuySell, int)
...
class TestRealWorldScenarios(unittest.TestCase):
    ...
    def test_multi_fill_order(self):
        ...
        fills = [
            {"FilledQuantity": 2, "AverageFillPrice": 5800.0},
            {"FilledQuantity": 5, "AverageFillPrice": 5800.15},
            {"FilledQuantity": 8, "AverageFillPrice": 5800.20},
        ]

        for filled_qty, avg_price in [(f["FilledQuantity"], f["AverageFillPrice"]) for f in fills]:
            order = OrderUpdate.model_validate(
                {
                    "Type": 301,
                    "ServerOrderID": "MULTI-FILL-001",
                    "FilledQuantity": filled_qty,
                    "AverageFillPrice": avg_price,
                }
            )
            self.assertEqual(order.FilledQuantity, filled_qty)
            self.assertEqual(order.get_avg_fill_price(), avg_price)
